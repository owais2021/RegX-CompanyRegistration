...
def fetch_dataset(organization_id):
    ...
    if response.status_code == 200:
        data = response.json()
        if data.get("success"):
            log.info(f"Datasets for organization '{organization_id}' retrieved successfully!")
            return data["result"]["results"]
        else:
            log.error(f"Error fetching datasets: {data.get('error')}")
    else:
        log.error(f"Failed to connect to CKAN API: {response.status_code}")
...
def get_json_resource_url(resources):
    ...
    for resource in resources:
        if resource.get("format", "").lower() == "json":
            log.info(f"Processing JSON resource: {resource.get('legalName')}")
            return resource.get("url")
    log.warning("No JSON resource found in the dataset!")
    return None
...
def download_and_parse_json(resource_url):
    ...
    if response.status_code == 200:
        ...
        log.error(f"Failed to download resource: {response.text}")
    return []
...
def save_to_single_file(data, file_path):
    ...
    log.info(f"All extracted data saved to {file_path}")
...
def main():
    ...
    for dataset in datasets_to_process:
        log.info(f"Processing dataset: {dataset.get('title')}")
        ...
        if extracted_names:
            ...
            log.info(f"Extracted legalName: {extracted_names}")

    if not all_extracted_data:
        log.warning("No legalName extracted.")
        return
    ...
    if connection:
        for entry in all_extracted_data:
            log.info(f"Attempting to insert: ocid={entry['ocid']}, legalName={entry['legalName']}")
